...
const datePicker = (d) =>
  createElement({
    tagName: 'span',
    props: { id: 'from-date-datePicker', style: { width: '240px' } },
    onMount: function () {
      document.getElementById(
        'from-date-datePicker',
      ).innerHTML = `<input type='text' style='display:none;' id='txt12345' name="${d.name || ''}" value="${
        d.value || ''
...
export const employmentTypeInput = (d) =>
  withLabel(Select({ options: d.options, value: d.value, ...d }), 'employment type')
...
export const fromBirthDateInput = (d) =>
  withLabel(createElement({ tagName: 'input', props: { ...d } }), 'from birth date')
...
export const languageLevelInput = (d) =>
  withLabel(createElement({ tagName: 'input', props: { ...d } }), 'language level')
