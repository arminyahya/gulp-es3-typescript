...
export default function uniqueidentifierType({ mode, value, name }: Props) {
  let component
  switch (mode) {
    case 'display':
    ...
    case 'filter':
    case 'wizard':
      component = createElement<HTMLInputElement>({ tagName: 'input', props: { value, style: { direction: 'ltr' } } })
  }
  return component
}
