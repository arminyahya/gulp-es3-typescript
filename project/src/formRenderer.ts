...
function formRenderer(structure: DynamicDataModel.FormGeneratorViewModel) {
	const designForm = [...structure.DesigendForm as any].map((control) => ({info: control ,component: lib.createElement({tagName: 'div', innerText: control.control.Code})}));
	...
	function findChildsAndAppendToParrent(root: any ) {
		 (designForm as any).filter((control) => control.info.parentId === root.info.id).forEach((child) => {
			root.component.appendChild(findChildsAndAppendToParrent(child).component)
		})
		return root
	}
